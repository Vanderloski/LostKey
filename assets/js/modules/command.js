...
export const command = async (command, devAction) => {
	...
	//MAKE GLOBAL VARIABLE SET TO LAST ERROR MESSAGE AND THEN CHECK IF NEXT PLAYER COMMAND EQUALS VARIABLE THEN SEND THIS MESSAGE
	/*if (command === "I DON'T UNDERSTAND" || command === "I DONT UNDERSTAND") {
		return { response: ["ARE YOU MOCKING ME?"] };
	} else {


		//IF ACTION FOUND
		if (commandObj.needClarification || commandObj.error || commandObj.syntaxError) {
			return commandObj;
		} else {
			//GET BEFORE EVENTS
			evtRes = await EVENT.event(commandObj.action, commandObj.object, 1);
			...
			if (commandObj.action && evtRes?.event_only !== 1) {
				if (!(commandObj.action.dev_action === 1 && devAction !== 1)) { //SKIP DEV ACTION COMMANDS SENT BY PLAYER
					//IF INVENTORY COMMAND
					if (commandObj.action.class === "INVENTORY") {
						if ($('#sidebar').css('display') === 'block' || $('#sidebar').css('display') === 'flex') {
							$('#sidebar').animate({
								width: '0%'
							}, () => {
								$('#sidebar').hide();
						...
						//GET ACTION BY TYPE
						if (commandObj.action.class === "MOVEMENT") {
							cmdRes = await MC.movement(commandObj.action, commandObj.object);
						} else if (commandObj.action.class === "ITEM") {
							cmdRes = await ITEM.item(commandObj.action, commandObj.object);
						} else if (commandObj.action.class === "DOOR") {
							cmdRes = await DOOR.door(commandObj.action, commandObj.object);
						} else if (commandObj.action.class === "PERSON") {
							cmdRes = await PERSON.person(commandObj.action, commandObj.object);
						} else if (commandObj.action.class === "SYSTEM") {
							cmdRes = await SYSTEM.system(commandObj.action, commandObj.object);
						}

						//CHECK FOR FLAVOR TEXT
						if (commandObj.action.flavor && parseInt(cmdRes.isValid) === 1) {
							cmdRes.response.unshift('///' + commandObj.action.flavor.replaceAll(' ', '_'));
						}
					}
				}
			}

			evtResAfter = await EVENT.event(commandObj.action, commandObj.object, 0);
			if (evtResAfter && evtResAfter.length > 0) {
				evtResAfter = evtResAfter[0];
			} else {
				evtResAfter = null;
			}
		}
	...
	//CHECK FOR PREP
	/*if (commandObj?.action && commandObj?.action?.syntaxFix) {
		syntaxFix = [('///' + commandObj.action.originalAction + ' ' + commandObj.object.object + '.').replaceAll(' ','_')];
	}

	if (!cmdRes && !evtRes && !evtResAfter) {
		if (!cmdRes && commandObj?.action && commandObj?.action?.fallback) {
			return { response: [commandObj.action.fallback] };
		} else {
			return { response: ["I DON'T UNDERSTAND."] };
		}
	} else if (cmdRes?.error || evtRes?.error) {
		return cmdRes || evtRes;
	} else {
		//SET PERSONS AND ITEMS TO FOUND AND SCENES TO VISITED
		await setSceneVisited();
		const newPersons = await setPersonEncounter();
		const newItems = await setItemEncounter();

		return {
			response: [...(syntaxFix), ...(evtRes?.response || []), ...(cmdRes?.response || []), ...(evtResAfter?.response || []), ...(newItems || []), ...(newPersons || [])],
			sceneTitle: cmdRes?.sceneTitle,
			inventory: cmdRes?.inventory,
			inventoryChange: cmdRes?.inventoryChange,
			setCommand: cmdRes?.setCommand
		}
	}*/
}
...
async function getObject(objectCheck) {
	...
	itemCheck = items.filter((it) => {
		...
		if ((it.scene === "INVENTORY" || it.scene === player.scene) && it.encountered === 1) {
			if (nameNoThe === objectNoThe || titleNoThe === objectNoThe) {
				return true;
			} else if (it?.alias) {
				for (let i = 0; i < it.alias.length; i++) {
					const aliasNoThe = removeThe(it.alias[i]);
					if (aliasNoThe === objectNoThe) {
						return true;
					}
				}
			} else {
	...
	doorCheck = doors.filter((d) => {
		...
		if (d?.scenes) {
			return d?.scenes.filter((dd) => {
				if (dd.scene === player.scene) {
					if (objectNoThe === "DOOR") {
						return true;
					} else if (nameNoThe === objectNoThe || titleNoThe === objectNoThe) {
						return true;
					} else {
						return false;
					}
	...
	//PERSONS
	personCheck = persons.filter((p) => {
		p.person_name = p.name;
		const nameNoThe = removeThe(p.name);
		const titleNoThe = removeThe(p.title);

		if (p.scene === player.scene) {
			if (nameNoThe === objectNoThe || titleNoThe === objectNoThe) {
				return true;
			} else if (p?.alias) {
				for (let i = 0; i < p.alias.length; i++) {
					const aliasNoThe = removeThe(p.alias[i]);
					if (aliasNoThe === objectNoThe) {
						return true;
					}
				}
	...
	return [...itemCheck, ...doorCheck, ...personCheck];
}
*/

/*if (!action || action.length === 0) {
		//IF ACTION NOT FOUND CHECK IF SCENE NAME
		const sceneName = scenes.filter((scene) => {
			if (scene?.exits) {
				return scene.exits.filter((exit) => {
					return exit.direction.filter((dir) => {
						//REMOVE ARTICLE "THE" FROM CHECK
						const dirNoThe = removeThe(dir);
						return dirNoThe === command;
					})[0];
				})[0];
			} else {
				return false;
		...
		//SET ACTION AND OBJECT FOR SCENENAME
